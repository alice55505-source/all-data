...
建立超過一週、完全沒有任何召會上傳過資料的房間會被自動清掉（Cloudflare Pages Functions 不支援排程工作，所以這是搭「建立新房間」這個動作順便做的清理，不是準時觸發，純粹是為了掃掉建立後沒人用的空房間；已經上傳過資料的房間不會被這樣清掉）。
...
- 「快速選擇」提供兩個常用預設：「全台兒童」（召會生活、小排 × 兒童 + 兒童主日）、「全台青職」（主日、家聚會（出訪+受訪）、小排、生命讀經 × 青職），點下去只是把打勾狀態改成該預設，仍需按「儲存並套用」才會存檔，按之前可以再自行調整
- 「清空全部」可以把所有勾選清空
